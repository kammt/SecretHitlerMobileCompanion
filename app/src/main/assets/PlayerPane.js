    ...
    constructor(players) {
		...
		this.pane.on('mousedown', (e) => {
			...
			$(document).on('mousemove touchmove', (e) => {
                ...
                let moveType = e.handleObj.type;

                let currentPos;

                if(e.handleObj.type === 'touchmove') {
                    currentPos = e.originalEvent.touches[0].clientX;
                } else {
                    currentPos = e.originalEvent.x;
                }

                //console.log(e);
                //console.log(`   X position: ${currentPos}`);
				this.offset(currentPos);
			});
		});

		this.pane.on('mouseup', (e) => {
			if (this.clickedOn) {
                ...
                //console.log(e);
                //console.log(`   X position: ${e.originalEvent.x}`);
        ...
        this.pane.on('touchstart', (e) => {
            $('body').addClass('stop-scrolling');
        })

        this.pane.on('touchend', (e) => {
            $('body').removeClass('stop-scrolling');
        })
